...
<script>
	...
	$('#btnAddFile').click(function() {
		if($('.actorFile').last().val() == '') {
			...
			let html = '<div><input type = "file" name = "actorFile" class = "actorFile"></div>';
			$('#fileDiv').append(html);
		}
	});
	
	$('#btnRemoveFile').click(function() {
		// 마지막 <input type = "file" name = "actorFile" class = "actorFile"> 제거
		// foreach(JQuery)
		//$('.actorFile').last().remove();
		if($('.actorFile').length == 0){ // class =actorFile 이 럾을때
